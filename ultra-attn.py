...
class Agent(nn.Module):
    ...
    def get_value(self, obs_info_pm, obs_info_all_vm, obs_info_edges, obs_info_num_steps, obs_info_num_vms):
        ...
        if self.model == "attn":
            return self.vm_net(obs_info_all_vm, obs_info_num_steps, obs_info_pm, obs_info_edges, num_vms_mask)[1]
        else:
...
if __name__ == "__main__":
    ...
    wandb_tags = [args.model, args.gym_id, 'same_optim', 'ult_attn', 'onehead', 'interleave']
    if args.normalize:
        wandb_tags.append('norm')
    if less_feature:
        wandb_tags.append('less_feature')
    if args.track:
    ...
    agent = Agent(vm_cand_model, params, args.model)  # , pm_cand_model, params, args.model)
