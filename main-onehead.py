...
if __name__ == "__main__":
    ...
    if args.track:
        wandb.init(entity="zhykoties",
                ...
                'eff_b_sz': args.accum_iter * args.num_minibatches},
                   # notes="",
                   tags=[args.model, args.gym_id, 'onehead', 'norm'] if args.normalize else [args.model, args.gym_id, 'onehead'],
                   save_code=True
                   )
    ...
    agent = Agent(vm_cand_model, params, args.model)
    ...
    for update in pbar:
        ...
        for epoch in range(args.update_epochs):
            ...
            for index, start in enumerate(range(0, args.batch_size, args.minibatch_size)):
                ...
                _, _, logprob, entropy, newvalue, _ = agent.get_action_and_value(
                    envs,
                    b_obs_pm[mb_inds],
                    b_obs_vm[mb_inds],
        ...
        if args.track:
            ...
            if (update + 1) % save_every_step == 0:
                utils.save_checkpoint({'global_step': global_step,
                                       'state_dict': agent.state_dict(),
                                       'optim_dict': vm_optim.state_dict()},
                                      global_step=global_step,
                                      checkpoint=f"runs/{run_name}")
